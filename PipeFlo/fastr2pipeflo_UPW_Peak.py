	...
	with open( data_file_path ) as csvfile:
		...
		for data_col in reader:
			...
			if "BYPASS" in fpoc_name:																#check to see if a UPW bypass valve
				...
				flow_value = data_col[lincount + 2]													#EDIT THIS VALUE DEPENDING ON WHAT COLUMN THE DEMANDS ARE IN
				...
				try:											 									#set environment to test values without aborting the rest of the script when errors are encountered
					...
					print( 'Did not find bypass_sizing_valve', sizing_valve_name ) 					#print problem to the output window
				...
				bypass_value = data_col[lincount + 2]
				
				try:
					p = doc.get_flow_demand( flow_demand_name )
				except RuntimeError:
					print('Did not find flow_demand', flow_demand_name)
					errors += 1
					continue
				...
				try:											 									#set environment to test values without aborting the rest of the script when errors are encountered
					...
					print( 'Did not find tool_bypass_valve', bypass_name ) 					#print problem to the output window
					errors += 1  																	#adds 1 to the error count - OPTIONAL
					continue																		#return to the top of the for loop and continue with the next row of data
				...
				try: 																				#set environment to test values
					...
					bypass_valve_flow = 0
					print( 'Invalid value type', bypass_name, bypass_value ) 					#print problem to the output window
					errors += 1
					continue 
